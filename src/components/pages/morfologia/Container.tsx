...
import { aberturaImagem, aplicacaoMascaraBinaria, contornoExternoImagem, contornoInternoImagem, dilatarImagem, erodirImagem, fechamentoImagem, gradienteImagem, transfomarBinario } from "./Operacao"
    ...
    const calcular = (label: any) => {
        ...
        switch (label) {
            case TiposTransformacaoBinaria.DILATACAO:
                ...
                const t = dilatarImagem(imagemBinaria, elementoEstruturante)
            //    console.log("t", t)
                setImagemTransformada(t)
                break
            case TiposTransformacaoBinaria.EROSAO:
                const erodida = erodirImagem(imagemBinaria, elementoEstruturante)
             //   console.log("t", erodida)
                setImagemTransformada(erodida)
                break
            case TiposTransformacaoBinaria.ABERTURA:
                const abertura = aberturaImagem(imagemBinaria, elementoEstruturante)
                console.log("t", abertura)
                setImagemTransformada(abertura)
                break
            case TiposTransformacaoBinaria.FECHAMENTO:
                const fechamento = fechamentoImagem(imagemBinaria, elementoEstruturante)
                setImagemTransformada(fechamento)
                break
            case TiposTransformacaoBinaria.CONTORNOEXTERNO:
                const externo = contornoExternoImagem(imagemBinaria, elementoEstruturante)
                setImagemTransformada(externo)
                break
            case TiposTransformacaoBinaria.CONTORNOINTERNO:
                const interno = contornoInternoImagem(imagemBinaria, elementoEstruturante)
                setImagemTransformada(interno)
                break
            case TiposTransformacaoBinaria.GRADIENTE:
                const gradiente = gradienteImagem(imagemBinaria, elementoEstruturante)
                setImagemTransformada(gradiente)
                break
